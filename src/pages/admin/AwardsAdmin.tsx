    ...
    <div className="space-y-10 pb-24">
      ...
      <div className="flex items-center gap-2 p-1.5 bg-stone-100 rounded-2xl w-fit">
        <button
          ...
          className={`px-6 py-3 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all ${activeTab === 'members' ? 'bg-white text-[#4F5BD5] shadow-lg' : 'text-stone-400'}`}
        >
          メンバー活動
        </button>
        <button
          onClick={() => setActiveTab('ranking')}
          className={`px-6 py-3 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all ${activeTab === 'ranking' ? 'bg-white text-[#D62976] shadow-lg' : 'text-stone-400'}`}
        >
          活動表彰
        </button>
      </div>

import { useState } from 'react';
import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import {
  ClipboardList,
  Compass,
  Sparkles,
  MapPin,
  Loader2,
  X,
  Shield,
  Crown,
  Menu,
  Home as HomeIcon,
  Calendar,
  Hash,
  GraduationCap,
  Heart,
  Globe,
  Mail,
  Phone,
  MessageCircle
} from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '../lib/supabase';
import { useAuthStore } from '../store/useAuthStore';
import { useAppStore } from '../store/useAppStore';
import { format } from 'date-fns';
import { clsx, type ClassValue } from 'clsx';
import { twMerge } from 'tailwind-merge';
import type { SurveyResponse as SurveyResponseType } from '../types/survey';

function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

function SectionTitle({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="flex items-center gap-4">
      <div className="w-1.5 h-12 rounded-full bg-gradient-to-b from-[#4F5BD5] to-[#D62976]" />
      <div>
        <h4 className="text-[22px] sm:text-[28px] font-black tracking-tighter text-stone-900">{title}</h4>
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-stone-400">{subtitle}</p>
      </div>
    </div>
  );
}

export default function Profile() {
  const { currentUser, currentRole } = useAuthStore();
  const { selectedYear } = useAppStore();
  const [isSidebarVisible, setIsSidebarVisible] = useState(false);
  const [activeRecordTab, setActiveRecordTab] = useState<'activities' | 'forms'>('activities');

  const { data: profileData, isLoading: isProfileLoading } = useQuery({
    queryKey: ['profile', currentUser?.id, selectedYear?.id],
    queryFn: async () => {
      if (!currentUser || !selectedYear) return null;
      const { data: user, error } = await supabase
        .from('users')
        .select('*, club_memberships(*)')
        .eq('id', currentUser.id)
        .eq('club_memberships.academic_year_id', selectedYear.id)
        .single();
      if (error) throw error;
      return user;
    },
    enabled: !!currentUser && !!selectedYear,
  });

  const activeMembership = profileData?.club_memberships?.[0] || null;

  const { data: appSettings } = useQuery({
    queryKey: ['app-settings'],
    queryFn: async () => {
      const { data, error } = await supabase.from('app_settings').select('*');
      if (error) throw error;
      return data.reduce((acc: any, curr) => {
        acc[curr.key] = curr.value;
        return acc;
      }, {});
    }
  });

  const isAdmin = currentRole && ['president', 'vice_president', 'treasurer', 'executive'].includes(currentRole);
  const isFullDisclosure = isAdmin || appSettings?.allow_profile_edit === true;

  const { data: historyData = [], isLoading: isHistoryLoading } = useQuery({
    queryKey: ['user-history', currentUser?.id, selectedYear?.id],
    queryFn: async () => {
      if (!currentUser || !selectedYear) return [];
      const { data, error } = await supabase
        .from('registrations')
        .select('*, activities!inner(*), attendance_records(*)')
        .eq('user_id', currentUser.id)
        .eq('activities.academic_year_id', selectedYear.id);
      if (error) throw error;
      return (data || []).sort((a: any, b: any) => new Date(b.activities.date).getTime() - new Date(a.activities.date).getTime());
    }
  });

  const { data: answeredSurveys = [], isLoading: isSurveyHistoryLoading } = useQuery({
    queryKey: ['user-survey-history', currentUser?.id, selectedYear?.id],
    queryFn: async () => {
      if (!currentUser || !selectedYear) return [];
      const { data, error } = await supabase
        .from('survey_responses')
        .select(`
          id,
          survey_id,
          user_id,
          submitted_at,
          updated_at,
          surveys!inner(
            id,
            title,
            description,
            status,
            response_mode,
            academic_year_id,
            activities(id, title, date, location)
          )
        `)
        .eq('user_id', currentUser.id)
        .eq('surveys.academic_year_id', selectedYear.id)
        .order('updated_at', { ascending: false });
      if (error) throw error;
      return (data || []) as Array<SurveyResponseType & { surveys?: any }>;
    },
    enabled: !!currentUser && !!selectedYear,
  });

  const { data: attendanceStats } = useQuery({
    queryKey: ['attendance-stats', currentUser?.id, selectedYear?.id],
    queryFn: async () => {
      if (!currentUser || !selectedYear) return { internal_count: 0, external_count: 0 };
      const { data, error } = await supabase.rpc('get_user_attendance_stats', {
        user_uuid: currentUser.id,
        academic_year_uuid: selectedYear.id
      });
      if (error) throw error;
      return data?.[0] || { internal_count: 0, external_count: 0 };
    },
    enabled: !!currentUser && !!selectedYear
  });

  if (isProfileLoading) {
    return (
      <div className="h-[calc(100vh-4.5rem)] flex items-center justify-center bg-[#FAFBFF]">
        <Loader2 className="w-8 h-8 text-indigo-600 animate-spin" />
      </div>
    );
  }

  if (!profileData) return null;

  const InfoRow = ({ label, value, icon: Icon, color, isVertical }: any) => (
    <div className={cn(
      'group flex py-2.5 border-b border-stone-50 transition-colors gap-2 overflow-visible',
      isVertical ? 'flex-col items-start' : 'items-center'
    )}>
      <div className="flex items-center gap-2.5 shrink-0 min-w-[90px]">
        <div className={`p-1.5 rounded-lg ${color} bg-opacity-10 text-opacity-100`}>
          <Icon className={`w-3.5 h-3.5 ${color.replace('bg-', 'text-')}`} />
        </div>
        <label className="text-[16px] font-bold text-stone-700 uppercase tracking-[0.05em] font-sans">{label}</label>
      </div>
      <div className="flex items-center gap-2 text-stone-900 w-full">
        {!isVertical && <span className="text-stone-600 font-black">:</span>}
        <span className={cn(
          'text-[16px] font-black tracking-tight font-sans break-all',
          label === 'LINE' ? 'text-[#06C755]' : 'text-stone-900',
          isVertical ? 'pl-9 text-[15px] opacity-80' : ''
        )}>
          {(value && value.toString().trim() !== '' && value !== 'null') ? value : '無'}
        </span>
      </div>
    </div>
  );

  return (
    <div className="h-[calc(100vh-4rem)] sm:h-[calc(100vh-4.5rem)] bg-[#F8F9FD] overflow-hidden relative font-sans text-stone-900">
      {isSidebarVisible && (
        <div
          onClick={() => setIsSidebarVisible(false)}
          className="lg:hidden fixed inset-0 bg-indigo-900/20 z-[40]"
        />
      )}

      <div className="h-full flex relative overflow-hidden">
        <aside
          className={`bg-white border-r border-stone-100 flex flex-col z-[50] shrink-0 overflow-hidden relative shadow-2xl lg:shadow-none transition-all duration-300 ${
            isSidebarVisible
              ? 'w-full lg:w-[440px] fixed lg:relative inset-y-0 left-0 max-w-[440px]'
              : 'w-0 fixed lg:w-0 lg:relative'
          }`}
        >
          <button
            onClick={() => setIsSidebarVisible(false)}
            className="absolute top-4 right-4 w-12 h-12 bg-white hover:bg-rose-50 text-stone-400 hover:text-rose-500 rounded-2xl flex items-center justify-center shadow-sm border border-stone-100 transition-colors active:scale-95 z-50"
          >
            <X className="w-6 h-6" strokeWidth={2.5} />
          </button>

          <div className="flex-1 flex flex-col p-8 lg:p-12 overflow-y-auto scrollbar-none">
            <div className="flex flex-col items-center mb-12">
              <div className="relative mb-6">
                <div className="w-24 h-24 rounded-[2.5rem] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shadow-2xl shadow-indigo-200">
                  <span className="text-3xl font-black">{profileData.full_name[0]}</span>
                </div>
                {activeMembership?.role === 'admin' && (
                  <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-amber-400 rounded-2xl flex items-center justify-center border-4 border-white shadow-lg">
                    <Crown className="w-5 h-5 text-white fill-white/20" />
                  </div>
                )}
              </div>
              <h2 className="text-[32px] font-black tracking-tighter leading-tight font-serif mb-2 text-center">
                {profileData.full_name}
              </h2>
              <p className="text-[12px] font-black text-indigo-500 tracking-[0.4em] uppercase opacity-70 text-center">
                {profileData.full_name_kana}
              </p>
            </div>

            {isFullDisclosure && (
              <div className="grid grid-cols-2 gap-4 mb-12">
                <div className="bg-indigo-50/50 border border-indigo-100/50 rounded-[2rem] p-5 flex flex-col items-center gap-2 transition-colors hover:bg-indigo-100/20 group">
                  <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-indigo-600">
                    <Sparkles className="w-5 h-5" />
                  </div>
                  <div className="text-center">
                    <p className="text-[22px] font-black text-indigo-900 leading-none">{attendanceStats?.internal_count || 0}</p>
                    <p className="text-[11px] font-black text-indigo-400 mt-2 uppercase tracking-widest text-[10px]">学内活動</p>
                  </div>
                </div>
                <div className="bg-orange-50/50 border border-orange-100/50 rounded-[2rem] p-5 flex flex-col items-center gap-2 transition-colors hover:bg-orange-100/20 group">
                  <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-orange-500">
                    <Compass className="w-5 h-5" />
                  </div>
                  <div className="text-center">
                    <p className="text-[22px] font-black text-orange-900 leading-none">{attendanceStats?.external_count || 0}</p>
                    <p className="text-[11px] font-black text-orange-400 mt-2 uppercase tracking-widest text-[10px]">学外活動</p>
                  </div>
                </div>
              </div>
            )}

            <div className="space-y-10">
              <div className="flex items-center gap-1 pb-2 border-b border-stone-100">
                <Shield className="w-4 h-4 text-indigo-400" />
                <span className="text-[16px] font-black text-stone-900 uppercase tracking-[0.2em] font-serif">基本情報</span>
              </div>

              <div className="flex flex-col gap-1">
                <InfoRow label="学籍番号" value={profileData.mssv} icon={Hash} color="bg-blue-500" />

                {isFullDisclosure && (
                  <>
                    <InfoRow
                      label="性別"
                      value={profileData.gender === 'Male' ? '男性' : profileData.gender === 'Female' ? '女性' : 'その他'}
                      icon={Heart}
                      color="bg-rose-500"
                    />
                    <InfoRow
                      label="学年"
                      value={activeMembership?.university_year === 0 ? '卒業生' : (activeMembership?.university_year ? `${activeMembership.university_year}年` : '無')}
                      icon={GraduationCap}
                      color="bg-purple-500"
                    />
                    <InfoRow
                      label="国籍"
                      value={profileData.nationality}
                      icon={Globe}
                      color="bg-emerald-500"
                    />
                  </>
                )}
              </div>

              {isFullDisclosure && (
                <motion.div
                  initial={{ opacity: 0, height: 0 }}
                  animate={{ opacity: 1, height: 'auto' }}
                  className="space-y-6 overflow-hidden"
                >
                  <div className="flex items-center gap-3 pt-6 pb-2 border-b border-stone-100">
                    <Mail className="w-4 h-4 text-indigo-400" />
                    <span className="text-[16px] font-black text-stone-900 uppercase tracking-[0.2em] font-serif">連絡先</span>
                  </div>

                  <div className="space-y-6">
                    <InfoRow label="連絡用メール" value={profileData.email} icon={Mail} color="bg-rose-500" isVertical={true} />
                    <InfoRow label="大学メール" value={profileData.university_email} icon={Mail} color="bg-indigo-500" isVertical={true} />
                    <InfoRow label="電話番号" value={profileData.phone} icon={Phone} color="bg-stone-600" />
                    <InfoRow label="LINE" value={profileData.line_nickname ? `@${profileData.line_nickname}` : null} icon={MessageCircle} color="bg-[#06C755]" />
                  </div>
                </motion.div>
              )}
            </div>
          </div>
        </aside>

        <main className="flex-1 flex flex-col bg-[#F8F9FD] relative overflow-hidden">
          <header className="px-5 sm:px-8 lg:px-16 py-4 sm:py-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6 border-b border-indigo-50 bg-white shrink-0 z-10 shadow-sm">
            <div className="flex w-full items-center justify-between gap-3 sm:hidden">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setIsSidebarVisible(true)}
                  className="flex h-10 w-10 items-center justify-center rounded-2xl border border-pink-100 bg-pink-50 text-[#D62976] shadow-sm active:scale-95"
                  aria-label="プロフィールメニューを開く"
                >
                  <Menu className="h-5 w-5" />
                </button>
                <Link
                  to="/"
                  className="flex h-10 w-10 items-center justify-center rounded-2xl border border-indigo-100 bg-indigo-50 text-[#4F5BD5] shadow-sm active:scale-95"
                  aria-label="ホームへ戻る"
                >
                  <HomeIcon className="h-4 w-4" />
                </Link>
              </div>

              <div className="flex h-9 items-center gap-2 rounded-full border border-stone-200 bg-white px-4 text-[12px] font-black text-stone-700 shadow-sm">
                <Calendar className="h-3.5 w-3.5 text-[#0f8b8d]" />
                <span>{selectedYear?.name}</span>
              </div>
            </div>

            <div className="hidden items-center gap-4 sm:flex [&_h3]:hidden">
              <button
                type="button"
                onClick={() => setIsSidebarVisible(true)}
                className="flex h-11 w-11 items-center justify-center rounded-2xl border border-pink-100 bg-pink-50 text-[#D62976] shadow-sm transition active:scale-95 hover:bg-pink-100"
                aria-label="プロフィールメニューを開く"
              >
                <Menu className="h-5 w-5" />
              </button>
                <h3 className="text-[24px] sm:text-[36px] font-black tracking-tighter font-serif leading-none text-stone-900">マイページ</h3>
            </div>

            <div className="hidden items-center gap-4 bg-white p-2 pr-4 sm:flex sm:pr-6 rounded-[1.5rem] sm:rounded-[2rem] border border-indigo-50 shadow-sm transition-all hover:shadow-md">
              <div className="w-10 h-10 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 font-black text-[12px]">26</div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Academic Year</span>
                <span className="text-[14px] font-black text-indigo-600">{selectedYear?.name}</span>
              </div>
            </div>
          </header>

          <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-16 py-8 sm:py-12 scrollbar-thin scrollbar-thumb-indigo-100">
            <div className="space-y-8">
              <div className="flex flex-wrap items-center gap-3 rounded-[2rem] bg-white/85 p-2 shadow-sm ring-1 ring-stone-100">
                <button
                  type="button"
                  onClick={() => setActiveRecordTab('activities')}
                  className={cn(
                    'inline-flex h-11 items-center gap-2 rounded-[1.25rem] px-4 text-[13px] font-black transition',
                    activeRecordTab === 'activities'
                      ? 'bg-[#1f2937] text-white shadow-[0_10px_24px_rgba(31,41,55,0.16)]'
                      : 'text-stone-500 hover:bg-stone-50 hover:text-stone-900'
                  )}
                >
                  <Sparkles className="h-4 w-4" />
                  活動履歴
                </button>
                <button
                  type="button"
                  onClick={() => setActiveRecordTab('forms')}
                  className={cn(
                    'inline-flex h-11 items-center gap-2 rounded-[1.25rem] px-4 text-[13px] font-black transition',
                    activeRecordTab === 'forms'
                      ? 'bg-[#1f2937] text-white shadow-[0_10px_24px_rgba(31,41,55,0.16)]'
                      : 'text-stone-500 hover:bg-stone-50 hover:text-stone-900'
                  )}
                >
                  <ClipboardList className="h-4 w-4" />
                  回答したフォーム
                </button>
              </div>

              {activeRecordTab === 'activities' ? (
              <section className="space-y-8">
                <SectionTitle title="活動履歴" subtitle="Activity Records" />

                {isHistoryLoading ? (
                  <div className="h-40 flex items-center justify-center">
                    <Loader2 className="w-10 h-10 text-indigo-600 animate-spin" />
                  </div>
                ) : historyData.length > 0 ? (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {historyData.map((reg: any) => {
                      const activity = reg.activities;
                      return (
                        <div
                          key={reg.id}
                          className="group bg-white border border-indigo-50/50 p-8 rounded-[3rem] transition-all duration-300 relative overflow-hidden"
                        >
                          <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-indigo-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                          <div className="flex flex-col h-full relative z-10">
                            <div className="flex items-start justify-between mb-8">
                              <div className="px-4 py-2 bg-stone-50 rounded-2xl border border-stone-100 transition-colors group-hover:bg-white group-hover:border-indigo-100">
                                <span className="text-[13px] font-black text-stone-900">{format(new Date(activity.date), 'yyyy.MM.dd')}</span>
                              </div>

                              {(() => {
                                let status = reg.attendance_status;
                                if (reg.attendance_records && reg.attendance_records.length > 0) {
                                  if (reg.attendance_records.some((r: any) => r.status === 'present')) status = 'present';
                                  else if (reg.attendance_records.some((r: any) => r.status === 'excused_absence')) status = 'excused_absence';
                                  else if (reg.attendance_records.some((r: any) => r.status === 'unexcused_absence')) status = 'unexcused_absence';
                                }

                                if (status === 'present') {
                                  return <span className="px-5 py-2 bg-emerald-50 text-emerald-600 text-[11px] font-black rounded-full border border-emerald-100 uppercase tracking-widest shadow-sm">出席</span>;
                                }
                                if (status === 'unexcused_absence') {
                                  return <span className="px-5 py-2 bg-rose-50 text-rose-600 text-[11px] font-black rounded-full border border-rose-100 uppercase tracking-widest shadow-sm">欠席</span>;
                                }
                                return <span className="px-5 py-2 bg-amber-50 text-amber-600 text-[11px] font-black rounded-full border border-amber-100 uppercase tracking-widest shadow-sm">確認中</span>;
                              })()}
                            </div>

                            <h4 className="text-[22px] font-black text-stone-900 group-hover:text-indigo-600 transition-colors uppercase tracking-tight leading-snug mb-6">
                              {activity.title}
                            </h4>

                            <div className="mt-auto flex items-center gap-4 text-[13px] font-bold text-stone-400">
                              <div className="flex items-center gap-2 bg-stone-50 px-4 py-2 rounded-xl group-hover:bg-indigo-50/50 group-hover:text-indigo-600 transition-all">
                                <MapPin className="w-4 h-4" />
                                <span>{activity.location}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center text-center max-w-sm mx-auto p-8">
                    <div className="w-32 h-32 rounded-[3.5rem] bg-white shadow-xl flex items-center justify-center mb-10 group">
                      <Sparkles className="w-12 h-12 text-rose-500/20 group-hover:text-rose-500 transition-colors duration-700" />
                    </div>
                    <h4 className="text-[26px] font-black tracking-tight mb-4 uppercase leading-tight">一緒に新しい活動に<br />参加しましょう！</h4>
                    <p className="text-stone-400 font-bold text-[14px]">
                      イベントに参加すると、ここにあなたの素敵な活動記録が刻まれます。
                    </p>
                  </div>
                )}
              </section>
              ) : (
              <section className="space-y-8">
                <SectionTitle title="回答したフォーム" subtitle="Answered Forms" />

                {isSurveyHistoryLoading ? (
                  <div className="h-40 flex items-center justify-center">
                    <Loader2 className="w-10 h-10 text-indigo-600 animate-spin" />
                  </div>
                ) : answeredSurveys.length > 0 ? (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                    {answeredSurveys.map((response) => {
                      const survey = Array.isArray(response.surveys) ? response.surveys[0] : response.surveys;
                      const activity = survey?.activities ? (Array.isArray(survey.activities) ? survey.activities[0] : survey.activities) : null;
                      if (!survey) return null;

                      return (
                        <Link
                          key={response.id}
                          to={`/surveys/${survey.id}`}
                          className="group bg-white border border-indigo-50/50 p-8 rounded-[3rem] transition-all duration-300 relative overflow-hidden hover:-translate-y-1"
                        >
                          <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-pink-50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                          <div className="relative z-10 flex flex-col h-full">
                            <div className="flex items-start justify-between mb-8 gap-4">
                              <div className="px-4 py-2 bg-stone-50 rounded-2xl border border-stone-100 group-hover:bg-white group-hover:border-pink-100 transition-colors">
                                <span className="text-[13px] font-black text-stone-900">
                                  {format(new Date(response.updated_at || response.submitted_at), 'yyyy.MM.dd')}
                                </span>
                              </div>
                              <span className="px-5 py-2 bg-indigo-50 text-indigo-600 text-[11px] font-black rounded-full border border-indigo-100 uppercase tracking-widest shadow-sm">
                                回答済み
                              </span>
                            </div>

                            <div className="flex items-start gap-4 mb-6">
                              <div className="w-14 h-14 rounded-[1.25rem] bg-gradient-to-br from-[#D62976]/10 to-[#4F5BD5]/10 text-[#D62976] flex items-center justify-center shrink-0">
                                <ClipboardList className="w-6 h-6" />
                              </div>
                              <div className="min-w-0">
                                <h4 className="text-[22px] font-black text-stone-900 group-hover:text-indigo-600 transition-colors tracking-tight leading-snug">
                                  {survey.title}
                                </h4>
                                {survey.description && (
                                  <p className="mt-2 text-[14px] font-bold text-stone-400 line-clamp-2">
                                    {survey.description}
                                  </p>
                                )}
                              </div>
                            </div>

                            <div className="mt-auto flex flex-wrap items-center gap-3 text-[13px] font-bold text-stone-400">
                              {activity?.title && (
                                <div className="flex items-center gap-2 bg-stone-50 px-4 py-2 rounded-xl group-hover:bg-indigo-50/50 group-hover:text-indigo-600 transition-all">
                                  <Sparkles className="w-4 h-4" />
                                  <span>{activity.title}</span>
                                </div>
                              )}
                              <div className="flex items-center gap-2 bg-stone-50 px-4 py-2 rounded-xl group-hover:bg-indigo-50/50 group-hover:text-indigo-600 transition-all">
                                <ClipboardList className="w-4 h-4" />
                                <span>フォームを確認</span>
                              </div>
                            </div>
                          </div>
                        </Link>
                      );
                    })}
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center text-center max-w-sm mx-auto p-8">
                    <div className="w-32 h-32 rounded-[3.5rem] bg-white shadow-xl flex items-center justify-center mb-10 group">
                      <ClipboardList className="w-12 h-12 text-indigo-300 group-hover:text-indigo-500 transition-colors duration-700" />
                    </div>
                    <h4 className="text-[26px] font-black tracking-tight mb-4 uppercase leading-tight">まだフォーム回答が<br />ありません</h4>
                    <p className="text-stone-400 font-bold text-[14px]">
                      回答したフォームはここからいつでも確認できます。
                    </p>
                  </div>
                )}
              </section>
              )}
            </div>
          </div>
        </main>
      </div>
    </div>
  );
}
